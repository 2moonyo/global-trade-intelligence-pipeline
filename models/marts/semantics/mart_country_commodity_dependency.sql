...
with commodity_trade_month as (
  ...
  from {{ ref('mart_reporter_commodity_month_trade_summary') }} as rc
),
...
commodity_share_metrics as (
  ...
  select
    ...
    {{ safe_divide(
      'greatest(ctm.commodity_trade_value_usd, 0)',
      'sum(greatest(ctm.commodity_trade_value_usd, 0)) over (partition by ctm.reporter_country_code, ctm.period, ctm.year_month)'
    ) }} * 100 as commodity_trade_share_pct
...
scored_commodities as (
  ...
  select
    ...
    {{ safe_divide('rc.cumulative_commodity_trade_value_for_share_usd', 'rc.reporter_trade_value_for_share_usd') }} * 100 as cumulative_commodity_share_pct,
