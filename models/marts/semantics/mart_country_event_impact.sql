...
with event_month_windows as (
  ...
  select
    ...
    bem.severity_weight,
    bem.is_global_event
  from {{ ref('bridge_event_month') }} as bem
),
event_month_chokepoints as (
  ...
  inner join {{ ref('bridge_event_chokepoint') }} as bec
...
reporter_event_exposure as (
  ...
  inner join {{ ref('mart_trade_exposure') }} as te
...
event_month_stress as (
  ...
  from event_month_chokepoints as emc
  left join {{ ref('stg_chokepoint_stress_zscore') }} as z
...
event_region_counts as (
  ...
  from {{ ref('bridge_event_region') }} as ber
  group by 1
),
event_metadata as (
...
event_profile as (
  ...
  from {{ ref('mart_event_impact') }} as mei
),
scored_reporter_events as (
  ...
  select
    ...
    {{ safe_divide('ree.event_exposed_trade_value_usd', 'ree.reporter_trade_value_usd') }} * 100 as event_exposure_pct,
