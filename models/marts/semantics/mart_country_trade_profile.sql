...
scored_profile as (
  ...
  select
    ...
    {{ safe_divide('rr.import_trade_value_usd', 'rr.total_trade_value_usd') }} * 100 as import_trade_share_pct,
    {{ safe_divide('rr.export_trade_value_usd', 'rr.total_trade_value_usd') }} * 100 as export_trade_share_pct,
    {{ safe_divide('rr.total_trade_value_usd - rr.previous_month_trade_value_usd', 'rr.previous_month_trade_value_usd') }} * 100 as trade_value_mom_change_pct,
    case
    ...
    case
      when {{ safe_divide('rr.export_trade_value_usd', 'rr.total_trade_value_usd') }} * 100 >= 55 then 'export_oriented'
      when {{ safe_divide('rr.import_trade_value_usd', 'rr.total_trade_value_usd') }} * 100 >= 55 then 'import_oriented'
      else 'mixed'
