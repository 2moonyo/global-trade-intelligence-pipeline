...
class QueryShape:
    partner_code: str = "all"  # "all" = partners for full global tracing 0 = "World"
    type_code: str = "C"
...
class ComtradeDataExtractor:
    ...
    def extract_one(self, reporter_code: str, period: str, commodity: str, flow: str, shape: QueryShape, timeout: int = 60) -> Dict:
        ...
        if str(shape.partner_code).lower() != "all":
            params["partnerCode"] = shape.partner_code
            
            # If specifically querying the World (0), ensure we don't double count secondary partners
            if str(shape.partner_code) == "0":
                params["partner2Code"] = 0
