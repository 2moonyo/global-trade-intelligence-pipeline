...
DEFAULT_LOCAL_DAILY_ROOT = PROJECT_ROOT / "data" / "silver" / "brent" / "brent_daily"
DEFAULT_LOCAL_MONTHLY_ROOT = PROJECT_ROOT / "data" / "silver" / "brent" / "brent_monthly"
DEFAULT_DAILY_GCS_PREFIX_PARTS = ("silver", "brent", "brent_daily")
DEFAULT_MONTHLY_GCS_PREFIX_PARTS = ("silver", "brent", "brent_monthly")
DAILY_PARTITION_FILENAME = "brent_daily.parquet"
MONTHLY_PARTITION_FILENAME = "brent_monthly.parquet"
LOGGER_NAME = "brent.load_bigquery"
LOG_DIR = PROJECT_ROOT / "logs" / "brent"
LOG_PATH = LOG_DIR / "load_brent_to_bigquery.log"
MANIFEST_PATH = LOG_DIR / "load_brent_to_bigquery_manifest.jsonl"
AUDIT_TABLE_NAME = "brent_load_audit"
STATE_TABLE_NAME = "brent_load_state"
...
def _bigquery_imports():
    try:
        ...
        raise ModuleNotFoundError(
            "google-cloud-bigquery is required for Brent BigQuery load operations."
        ) from exc
    ...
    *,
    ...
    for uri in candidate_uris:
        ...
        if blob_metadata is None:
            raise FileNotFoundError(f"Expected GCS object for Brent load source {uri}")
...
def load_brent(
    *,
    ...
    run_id = build_run_id("brent_load_bigquery")
    ...
    manifest_entry: dict[str, object] = {
        "run_id": run_id,
        "asset_name": "load_brent_to_bigquery",
        "dataset_name": "brent",
        "status": "running",
    ...
    try:
        logger.info("Starting Brent BigQuery load run_id=%s", run_id)
        logger.info("Step 1/4 Discover candidate Brent parquet partitions")
        ...
        if not daily_candidate_uris:
            raise FileNotFoundError(f"No Brent daily parquet partitions found for prefix {resolved_daily_prefix}")
        if not monthly_candidate_uris:
            raise FileNotFoundError(f"No Brent monthly parquet partitions found for prefix {resolved_monthly_prefix}")
        ...
        if not filtered_daily_uris and not filtered_monthly_uris:
            raise FileNotFoundError("No Brent partition files matched the requested year-month filters.")
        ...
        selected_year_months_values = sorted(set(selected_daily_months) | set(selected_monthly_months))
        skipped_year_months_values = sorted(set(skipped_daily_months) & set(skipped_monthly_months))

        manifest_entry["selected_year_months"] = selected_year_months_values
        manifest_entry["skipped_loaded_year_months"] = skipped_year_months_values
        manifest_entry["daily_selected_file_count"] = sum(
            len(daily_state[month].source_uris) for month in selected_daily_months
        )
        manifest_entry["monthly_selected_file_count"] = sum(
            len(monthly_state[month].source_uris) for month in selected_monthly_months
        )

        summary["selected_year_months"] = selected_year_months_values
        summary["skipped_loaded_year_months"] = skipped_year_months_values
        summary["daily_selected_file_count"] = manifest_entry["daily_selected_file_count"]
        summary["monthly_selected_file_count"] = manifest_entry["monthly_selected_file_count"]
        summary["daily_gcs_source_uris"] = [
            uri for month in selected_daily_months for uri in daily_state[month].source_uris
        ][:20]
        summary["monthly_gcs_source_uris"] = [
            uri for month in selected_monthly_months for uri in monthly_state[month].source_uris
        ][:20]

        logger.info(
            "Selected Brent months daily=%s monthly=%s skipped=%s",
            selected_daily_months,
            selected_monthly_months,
            skipped_year_months_values,
        )
        ...
        logger.info("Step 3/4 Submit Brent daily and monthly load jobs")
        _submit_load_if_needed(
            ...
            clustering_fields=["benchmark_code"],
            location=config.gcp_location,
            client=client,
            bigquery=bigquery,
        )
        ...
        logger.info("Step 4/4 Read final Brent row counts")
        output_rows = {
            "brent_daily": client.get_table(daily_table_id).num_rows,
            "brent_monthly": client.get_table(monthly_table_id).num_rows,
        }
        finished_at = datetime.now(timezone.utc)
        ...
        append_manifest(MANIFEST_PATH, manifest_entry)
        logger.info(
            "Finished Brent BigQuery load run_id=%s daily_rows=%s monthly_rows=%s duration_s=%.3f",
            run_id,
            output_rows["brent_daily"],
            output_rows["brent_monthly"],
        ...
        if client is not None and audit_table_ready:
            try:
                ...
                logger.exception("Failed to write Brent BigQuery audit row during failure handling run_id=%s", run_id)
        append_manifest(MANIFEST_PATH, manifest_entry)
        logger.exception("Brent BigQuery load failed run_id=%s", run_id)
        raise


def main() -> None:
    parser = argparse.ArgumentParser(description="Load Brent daily and monthly silver parquet from GCS into BigQuery raw tables.")
    parser.add_argument("--source", choices=("gcs", "local"), default="gcs")
    parser.add_argument("--local-daily-root", default=str(DEFAULT_LOCAL_DAILY_ROOT))
    parser.add_argument("--local-monthly-root", default=str(DEFAULT_LOCAL_MONTHLY_ROOT))
    parser.add_argument("--daily-gcs-prefix", default=None)
    parser.add_argument("--monthly-gcs-prefix", default=None)
    parser.add_argument("--daily-table-name", default="brent_daily")
    parser.add_argument("--monthly-table-name", default="brent_monthly")
