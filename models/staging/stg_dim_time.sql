...
with observed_months as (
  ...
  -- Fallback for feeds that populate period (YYYYMM) but omit/misformat year_month.
  select distinct
    {{ month_start_from_year_month("substr(" ~ cast_string('period') ~ ", 1, 4) || '-' || substr(" ~ cast_string('period') ~ ", 5, 2)") }} as month_start_date
  from {{ source('raw', 'comtrade_fact') }}
